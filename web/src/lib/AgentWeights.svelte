    ...
    <div class="weight-list">
      {#each analysis.choices as choice, index (index)}
        <div class="weight-row" class:best={choice.kind === analysis.choice.kind && choice.tile === analysis.choice.tile}>
          <span class="choice-label">{#if choice.tile}<Tile tile={choice.tile} size="tiny" />{/if}{choice.label}</span>
...
<style>
  ...
  .weight-row { display: flex; align-items: center; gap: 8px; padding: 5px 0; border-top: 1px solid #ffffff15; font-size: .82rem; }

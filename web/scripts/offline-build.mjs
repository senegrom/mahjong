...
export async function buildOffline(root) {
  ...
  const entries = await Promise.all(files.sort().map(async file => {
    ...
    return { url, hash: createHash('sha256').update(data).digest('hex'), bytes: data.length, group: ai ? 'ai' : 'core' };
  }));
  ...
  if (hasModel) for (const name of ['ort-wasm-simd-threaded.wasm', 'ort-wasm-simd-threaded.mjs']) {

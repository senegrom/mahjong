...
for (const [name, tile, label, status, sourceName, crop, notes] of definitions) {
  ...
  // Lossless WebP keeps the self-contained preview compact as the set grows.
  const previewRaster = execFileSync('convert', [path.join(out, png), '-define', 'webp:lossless=true', '-quality', '100', 'WEBP:-'], { maxBuffer: 16 * 1024 * 1024 });
  const previewSvg = svgText.replace(/data:image\/png;base64,[^"]+/, `data:image/webp;base64,${previewRaster.toString('base64')}`);
  previews.push({ name, label, status, src: `data:image/svg+xml;charset=utf-8,${encodeURIComponent(previewSvg)}` });
}
...
function draw(){const names=['Man1','Man2','Man3','Man5','Man6','Man7','Man9','Pin4','Pin6','Pin9','Sou1','Ton','Nan','Pei'];rack.replaceChildren(...names.map(name=>picture(byName[name])));table.style.width=document.getElementById('width').value+'px';requestAnimationFrame(updateSize)}

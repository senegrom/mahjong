...
<footer>This hand is a visual sample. The approved character tiles use Dance B for 1 and 2 characters, Jazz A for 3 characters, Interlock C with an ivory-white 四 for 4 characters and Cut-out C for 7 characters, all on deep purple. White dragon uses the approved C/C1 blend, with a pearly reveal when it is dora. Raw PNG crops retain their source resolution; each SVG provides the game's 300 × 400 canvas.</footer>
</main><script>
...
function draw(){const names=['Man1','Man2','Man3','Man4','Man7','Pin4','Pin6','Pin7','Pin9','Sou1','Ton','Nan','Shaa','Pei'];rack.replaceChildren(...names.map(name=>picture(byName[name])));table.style.width=document.getElementById('width').value+'px';requestAnimationFrame(updateSize)}

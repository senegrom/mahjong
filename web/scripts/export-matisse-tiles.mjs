...
for (const [name, tile, label, status, sourceName, crop, notes] of definitions) {
  ...
  previews.push({ name, label, status, src: `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svgText)}` });
}
...
function draw(){const names=['Man1','Man2','Man3','Man7','Man9','Pin3','Pin4','Pin6','Pin7','Pin9','Sou1','Ton','Shaa','Pei'];rack.replaceChildren(...names.map(name=>picture(byName[name])));table.style.width=document.getElementById('width').value+'px';requestAnimationFrame(updateSize)}

...
// Lossless extraction of Carl's selected artwork. K is excluded; L replaces F.
...
const manifest = {
  ...
  approval: 'Carl approved all except K and requested deployment. The later L is the active white dragon; F remains a study.',
  fallback: 'classic',
...
writeFileSync(path.join(root, 'web/src/lib/van-gogh-faces.js'), `// Generated by web/scripts/export-van-gogh-tiles.mjs. K excluded; L replaces F.\nexport const VAN_GOGH_APPROVED = Object.freeze(${JSON.stringify(approved)});\n`);
...
const hand = [...approved, '1z', '2z', '6z', '8m'].map(tile => {
...
<body><main><a href="../../">← Mahjong</a><h1>Van Gogh</h1><p>Ten approved faces. Choose <strong>Options → Tile face → Van Gogh</strong> in the game. The remaining tiles use Classic artwork.</p><h2>Approved artwork</h2><div class="gallery">${gallery}</div>
...
<p class="notes">The last four tiles use Classic artwork. East wind K is excluded. White dragon L keeps its pale painted dragon under the normal dora ring and foil sheen.</p><p class="notes">The exact selected pixels are preserved in lossless PNG crops. <a href="manifest.json">Export manifest</a></p></main><script>document.getElementById('width').addEventListener('change',event=>{document.getElementById('rack').style.width=event.target.value+'px'});</script></body></html>\n`);

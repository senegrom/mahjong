...
// Lossless extraction of the selected artwork, including Almond Branches for 3m.
...
const manifest = {
  ...
  approval: 'Carl approved A–E, G–J and L, excluding K, then selected East A and North B. On 13 September 2026 he selected Almond Branches (Characters A) to replace D for 3m and requested deployment. Night Cafe (Characters B) for 2m and Cypress Fields (Characters C) for 4m are also approved for deployment. The later L is the active white dragon; F and D remain studies.',
  fallback: 'classic',
...
writeFileSync(path.join(root, 'web/src/lib/van-gogh-faces.js'), `// Generated by web/scripts/export-van-gogh-tiles.mjs. Characters A replaces D; Characters B (2m), Characters C (4m), East A and North B included; K excluded; L replaces F.\nexport const VAN_GOGH_APPROVED = Object.freeze(${JSON.stringify(approved)});\n`);
...
<p class="notes">The last two tiles use Classic artwork. East wind K is excluded. White dragon L keeps its pale painted dragon under the normal dora ring and foil sheen.</p><p class="notes">The exact selected pixels are preserved in lossless PNG crops. <a href="manifest.json">Export manifest</a></p></main><script>document.getElementById('width').addEventListener('change',event=>{document.getElementById('rack').style.width=event.target.value+'px'});</script></body></html>\n`);

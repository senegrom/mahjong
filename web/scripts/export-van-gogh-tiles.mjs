...
// Lossless extraction of the selected artwork. East A and North B are the approved winds.
...
const manifest = {
  ...
  approval: 'Carl approved A–E, G–J and L, excluding K. He then selected Blazing Dawn (East A) for East and the corrected Wind Ribbons companion (North B) for North and requested deployment. The later L is the active white dragon; F remains a study.',
  fallback: 'classic',
...
writeFileSync(path.join(root, 'web/src/lib/van-gogh-faces.js'), `// Generated by web/scripts/export-van-gogh-tiles.mjs. East A and North B included; K excluded; L replaces F.\nexport const VAN_GOGH_APPROVED = Object.freeze(${JSON.stringify(approved)});\n`);
...
const hand = [...approved, '2z', '6z'].map(tile => {
...
<body><main><a href="../../">← Mahjong</a><h1>Van Gogh</h1><p>${approved.length} approved faces, including Blazing Dawn for East and Wind Ribbons for North. Choose <strong>Options → Tile face → Van Gogh</strong> in the game. The remaining tiles use Classic artwork.</p><h2>Approved artwork</h2><div class="gallery">${gallery}</div>

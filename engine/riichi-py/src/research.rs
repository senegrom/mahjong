...
fn research_fork(arena: &Arena, rows: Vec<usize>) -> PyResult<Arena> {
        ...
        || arena.imagined.iter().any(|worlds| !worlds.is_empty())
    {
        return Err(PyValueError::new_err(
            "finish outstanding search before snapshotting",
        ));
    }
...
fn research_context(arena: &Arena) -> Vec<Vec<i64>> {
    arena
        ...
        .map(|seat| {
            let mut row = vec![
                seat.hand.round.index() as i64,
                seat.hand.kyoku as i64,
                seat.table.dealer as i64,
                seat.hand.counters as i64,
                seat.hand.riichi_sticks as i64,
                seat.hand.discards_made as i64,
                seat.pending()
                    .map(|w| seat.table.player_at(w) as i64)
                    .unwrap_or(-1),
                match seat.hand.phase {
                    Phase::Act => 0,
                    Phase::CallWindow => 1,
                    Phase::Draw => 2,
                    Phase::Over => 3,
            ...
            for player in 0..4 {
                row.push(i64::from(
                    seat.hand.players[seat.table.seat_of(player).index()].has_riichi(),
                ));
            }
            row
        })
...
#[pyfunction]
fn research_danger<'py>(
    arena: &Arena,
    py: Python<'py>,
) -> (Bound<'py, PyBytes>, Bound<'py, PyBytes>) {
    let blocks: Vec<(Vec<u8>, Vec<f32>)> = py.detach(|| {
        arena
            .seats
            .par_iter()
            .map(|seat| {
                ...
                if seat.finished
                    || !seat.asking.is_empty()
                    || !matches!(seat.hand.phase, Phase::Act)
                {
                    return (mask, labels);
                }
                ...
                for action in seat.hand.legal_actions() {
                    ...
                    for distance in 1..4 {
                        ...
                        if !offered
                            .iter()
                            .any(|(who, calls)| *who == opponent && calls.contains(&Call::Ron))
                        {
                            continue;
                        }
    ...
    for (mask, label) in blocks {
        masks.extend(mask);
        labels.extend(label);
    }
    (
        PyBytes::new(py, &masks),
        PyBytes::new(py, bytemuck_cast(&labels)),
    )
}
    ...
    arena
        ...
        .map(|seat| {
            if seat.hands_done != 0 || seat.hand.discards_made != 0 || seat.hand.kyoku != 1 {
                return Err(PyValueError::new_err(
                    "initial counts must be captured before play",
                ));
            }
            let hands = seat
                .hand
                ...
                .find_map(|event| match event {
                    mjai::Event::StartKyoku { hands, .. } => Some(hands),
                    _ => None,
                })

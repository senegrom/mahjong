...
fn libriichi(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // Installed as a package by maturin, this module is `libriichi.libriichi`
    // and its submodules would be registered under that; everything that
    // imports them knows the short name, so that is the prefix they get.
    ...
    follow::register_module(py, name, m)?;

...
This follow-up is reconciled against `44fd950223e4e2929c8799f0587fe61e87d658cd`.
...
**Remaining search limitation:** the Mortal bridge does not yet reconstruct a
new hand and seating after an imagined hand boundary. Search calls encountering
such a nonterminal continuation fail explicitly. This repair does not claim full
...
The actual exploration coefficient is recorded per decision. A reach declaration
can use the policy/uniform mixture while the following tile decision remains
unmixed. Combined PPO uses those same per-decision coefficients in its likelihood
ratio and drift guard. At unchanged weights the ratio is one, including with
exploration. Outcome targets already describe continuations under this behaviour;
raw-policy entropy and the optional starting-policy leash remain regularizers.

...
new = "await p.click('.settings-trigger');await p.click('.options summary');await p.click('.option-fields input[type=checkbox]');await p.waitForFunction(()=>!document.querySelector('.copy-count'));assert.equal(await p.$('.copy-count'),null);"
if text.count(old) != 1:

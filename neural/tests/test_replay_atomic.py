...
class ReplayTests(unittest.TestCase):
    ...
    def test_failed_batch_or_manifest_publication_keeps_the_old_generation(self):
        ...
        for target in ('batch', 'manifest'):
            with self.subTest(target=target), tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as folder:
...
'''
        for phase, expected in [('before', 1), ('after', 2)]:
            with self.subTest(phase=phase), tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as folder:

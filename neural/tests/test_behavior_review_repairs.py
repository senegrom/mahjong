...
from neural.behavior import action_log_prob
...
class BehaviourTests(unittest.TestCase):
    ...
    def test_unchanged_exploring_policy_has_unit_ratios_and_zero_drift(self):
        ...
        actions,recorded,_=selfplay.explore(logits,legal,.25,np.random.default_rng(9))
        recalculated=action_log_prob(logits,legal,actions,torch.full((400,),.25))
        torch.testing.assert_close((recalculated-recorded).exp(),torch.ones(400),atol=1e-6,rtol=1e-6)
        guard=PolicyDrift(.02);self.assertFalse(guard.check(recorded,recalculated))
    ...
    def test_zero_coefficient_preserves_raw_likelihood_exactly(self):
        ...
        self.assertTrue(torch.equal(action_log_prob(logits,legal,actions,0.),expected))
    ...
    def test_both_reach_stages_record_their_actual_coefficients(self):
        ...
        reread=action_log_prob(logits,torch.from_numpy(records.masks),torch.from_numpy(records.actions),
                              torch.from_numpy(records.epsilon))
